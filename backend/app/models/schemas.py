...
class PartyInfo(BaseModel):
  """同行人数信息"""
  adults: int = Field(..., description="成人数量", ge=0, le=20, example=2)
  children: int = Field(default=0, description="儿童数量", ge=0, le=10, example=1)
  old: int = Field(default=0, description="老年人数量", ge=0, le=10, example=1)
  total: int = Field(..., description="总人数", ge=0, le=30, example=4)
  companion_type : str = Field(default = "other", description="同行类型（家庭、朋友、情侣等）",example="family")

  @model_validator(model = 'after')
  def validate_total(self):
    """total 必须等于 adults + children + old"""
    e_total = self.adults + self.children + self.old
    if self.total != e_total:
      raise ValueError(f"total 必须等于 adults + children + old, 但 total={self.total}, adults={self.adults}, children={self.children}, old={self.old}")  
    else:
      return self
...
class Location(BaseModel):
  """地理位置"""
  latitude: float = Field(..., description="纬度", ge=-90, le=90, example=39.9042)
  longitude: float = Field(..., description="经度", ge=-180, le=180, example=116.4074)
