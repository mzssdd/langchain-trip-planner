...
class PartyInfo(BaseModel):
    ...
    children: int = Field(default=0, description="儿童数量", ge=0, le=20, example=0)
    elders: int = Field(default=0, description="老人数量", ge=0, le=20, example=0)
    total: int = Field(..., description="同行总人数", ge=1, le=30, example=2)
    companion_type: str = Field(default="other", description="同行类型", example="couple")

    @model_validator(mode="after")
    def validate_total(self):
        """total 必须显式等于分项人数之和。"""
        expected_total = self.adults + self.children + self.elders
        if self.total != expected_total:
            raise ValueError(f"total必须等于adults+children+elders: expected={expected_total}, got={self.total}")
        if expected_total <= 0:
            raise ValueError("同行总人数必须大于0")
        return self
